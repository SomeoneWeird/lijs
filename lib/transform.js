...
export default function transformAST (ast) {
  ...
  function getUnusedVariable () {
    ...
    return ~variables.indexOf(variables) ? getUnusedVariable() : variable
  }
  ...
  const visitor = {
    ...
    IfStatement: function (node, parent) {
      ...
      out.consequent = {
        type: 'BlockStatement',
        body: node.pass.map(function (child) {
          return visit(child, node)
        })
      }

      out.alternate = node.fail ? {
        type: 'BlockStatement',
        body: node.fail.map(function (child) {
          return visit(child, node)
        })
      } : null
